...
const COUNTRIES = [
  ...
  { key: "indonesia", name: "인도네시아", flag: "🇮🇩", url: "indonesia/" },
];

// ================================================================
// ★★★ 메인 페이지 국가 카드 커버 이미지 직접 수정 ★★★
//   원하는 이미지 경로를 입력하세요.
//   비워두면 가장 최근 게시물 썸네일이 자동으로 채워집니다.
// ================================================================
...
function initCountries(allPosts) {
  ...
  COUNTRIES.forEach((c) => {
    ...
    const thumb = COUNTRY_COVERS[c.key] || posts[0]?.thumb || "";
  ...
  input?.addEventListener("input", () => {
    ...
    debounce = setTimeout(() => {
      ...
      const hits = allPosts
        ...
        .slice(0, 20);
      if (!hits.length) {
        results.innerHTML = `<p class="search-no-result">'${input.value}' 검색 결과가 없습니다</p>`;
        return;
      }
      results.innerHTML = "";
      hits.forEach((item) => {
